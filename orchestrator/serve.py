...
async def serve(
    ...
    *,
    ...
    The loop holds the Run controller lease for its whole lifetime, renewing it
    in the background, and exits (releasing the lease) when it loses ownership.
    Each cycle reclaims expired assignment leases, dispatches ready tasks,
    sweeps cancel requests, and reconciles agent pools to the configured
    target count. The loop stops cleanly on ``stop_event`` or after
    ``max_ticks`` cycles.
    """
    if interval <= 0:
