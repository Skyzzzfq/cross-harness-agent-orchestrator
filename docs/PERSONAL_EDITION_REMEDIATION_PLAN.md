...
编制日期：2026-09-11。状态：R0、R1、R2 已验收；R3–R8 待实施。
...
|---|---|---|---|---|
| R0 基线与能力 | **完成** | 待单独提交 | `docs/PERSONAL_EDITION_REMEDIATION_REPORT.md`；备份恢复通过；311/310/1；能力表与 Run 6 样本 | — |
| R1 角色与计划 | **完成** | 待本轮提交后回填 | Fake 计划/批准/幂等通过；真实 Codex 合法计划未验证，见整改报告 | 依赖 R0 |
| R2 四区与隔离 | **完成** | 待本轮提交后回填 | 四区/副本/scope 对账通过；真实后端权限未重测，见整改报告 | 依赖 R1 |
| R3 移交与聚合 | 待开始 | — | — | 依赖 R2 |
| R4 验证与交付 | 待开始 | — | — | 依赖 R3 |
| R5 消息与预算 | 待开始 | — | — | 依赖 R4 |
| R6 对话工作台 | 待开始 | — | — | 依赖 R5 |
